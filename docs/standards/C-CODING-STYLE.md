...
- All compiling and debugging happens inside docker xcc-z80-idp container on docker hub - https://hub.docker.com/r/wischner/xcc-z80-idp
...
- `bin/` is the distribution root: `bin/loader/`, `bin/lib/`, and
  `bin/include/`.
- `tests/dump/` contains object files, maps, listings, tests, and all other
...
- Project source files may live in `src/` or in a library directory
  under `lib/`.
- Intermediate build artifacts and test scratch files must go below
  `tests/dump/`; distributable programs and disk images go under `bin/`.
- Never create a root-level `build/`, `Testing/`, or temporary directory.
...
- All intermediate build outputs must go below `tests/dump/` only.
- Do not create build or testing directories at repository root.
